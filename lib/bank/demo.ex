defmodule Bank.Demo do
  ...
  # intent. The runtime pipeline writes simulation reports as part of
  # the decision flow (#90), but the seed creates intents directly via
  # `upsert_intent/4` and so bypasses that pipeline. Without a row
  # here, the `/sandbox` checklist's simulate step renders incomplete
  # on a fresh seed and `mix bank.sandbox.smoke` fails its `simulate`
  # check — both surfaces depend on a SimulationReport existing for
  # decided/planned/blocked intents.
    ...
    case Repo.one(
      ...
      nil ->
        ...
        |> SimulationReport.changeset(%{
          intent_id: intent.id,
          provider: "sandbox-stub",
          provider_trace_ref: "sandbox-" <> intent.id,
          chain: @chain,
          asset: @asset,
